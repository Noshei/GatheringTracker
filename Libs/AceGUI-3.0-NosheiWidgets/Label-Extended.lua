...
Widget is an extension of the Label Widget from AceGUI-3.0
We've hijacked ImageCoords from AceConfDisplay to be able to extend the new functions to AceConfig
    SetImage has been updated to only support SetAtlas
    the ImageCoords table has been repurposed to be used for SetVertexColor on the image
This Lib is not for general use outside of my own addons.
-------------------------------------------------------------------------------]]
local Type, Version = "NW_Label", 1
...
local methods = {
    ...
    ["SetImage"] = function(self, path, imageType, color)
        local image = self.image
        ...
        if image:GetTexture() then
            ...
            if color then
                image:SetVertexColor(unpack(color))
            end
        else
            self.imageshown = nil
        end
